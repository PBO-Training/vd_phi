...
export class NavContentComponent implements OnInit, AfterViewInit {
  ...
  constructor(public nav: NavigationItem, private zone: NgZone, private location: Location, private storageService: StorageService) {
    this.flatConfig = NextConfig.config;
    this.windowWidth = window.innerWidth;
    //  this.navigation = this.nav.get();
    this.navigation = this.storageService.getMenu();
  ...
  fireOutClick() {
    ...
    let parent_current_url = '';
    if (current_url && current_url.split('/').length > 1) {
      parent_current_url = '/' + current_url.split('/')[1];
    }
    let parLink;
    let parEle;
    if (parent_current_url !== '') {
      parLink = 'a.nav-link[ href=\'' + parent_current_url + '\' ]';
      parEle = document.querySelector(parLink);
    }
    const ele = document.querySelector(link);
    if (ele !== null && ele !== undefined) {
      const parent = ele.parentElement;
      ...
      if (parEle !== null && parEle !== undefined) {
        const parent = parEle.parentElement;
        const up_parent = parent.parentElement.parentElement;
        let last_parent = up_parent.parentElement;
