...
import { Vacation } from './vm002001-entity';
import { VM002001Service } from './vm002001.service';
import {
...
@Component({
  selector: 'app-vm002001',
  templateUrl: './vm002001.component.html',
  styleUrls: ['./vm002001.component.scss']
})
export class VM002001Component implements OnInit {

  selectDepartments: any[] = [];
  selectTypes: any[] = [];
  selectOptions: any[] = [];
  selectStatus: any[] = [];
  vacationReceiveList: Vacation[];
  ...
  constructor(
    ...
    private vm002001Service: VM002001Service,
    ...
    setTimeout(() => {
      this.initDataVacation(null, this.localStorage.getPageSize(), this.formBaseReceive);
    }, 1000);
  ...
  initDataLoad() {
    this.vm002001Service.vm002001InitList().subscribe(data => {
      if (data.error === null) {
  ...
  searchReceiveList() {
    ...
    this.formBaseReceive.value.vacationTypeID = this.valueOldForReceive.vacationTypeID;
    this.formBaseReceive.value.vacationOptionID = this.valueOldForReceive.vacationOptionID;
    this.formBaseReceive.value.vacationStatusID = this.valueOldForReceive.vacationStatusID;

    let startDate = this.parseDateToMonth(this.valueOldForReceive.startDate);
    if (startDate) {
      startDate.setHours(type.TimeDefault.startHourAM);
      startDate.setMinutes(type.TimeDefault.startMinAM);
    }
    let endDate = this.parseDateToMonth(this.valueOldForReceive.endDate);
    if (endDate) {
      endDate.setHours(type.TimeDefault.endHourPM);
      endDate.setMinutes(type.TimeDefault.endMinPM);
    }
    this.formBaseReceive.value.startDate = this.setDefaultTime(startDate);
    this.formBaseReceive.value.endDate = this.setDefaultTime(endDate);
  ...
  initDataVacation(pageN: any, pSize: any, form: FormGroup) {
    ...
    if (this.formSearch.valid) {
      ...
      this.vm002001Service.vm002001searchVacation(formData).pipe(takeUntil(this.unsubscribe$)).subscribe(datacheck => {
        if (datacheck.error === null) {
          this.vacationReceiveList = datacheck.content.data;
  ...
  onSort({ column, direction }: SortEvent) {
    ...
    this.column = column;
    this.vacationReceiveList = [...this.vacationReceiveList].sort((a, b) => {
      let res: number;
      switch (column) {
        case 'employeeName':
  ...
  onAddNewVacationTypeID(event) {
    ...
    this.valueOldForReceive = JSON.parse(JSON.stringify(this.formSearch.value));
    let newVacationTypeIDList: number[] = this.valueOldForReceive.vacationTypeID;
  ...
  onAddNewVacationOptionID(event) {
    const itemValue: number = Number(event);
    this.valueOldForReceive = JSON.parse(JSON.stringify(this.formSearch.value));
    let newVacationOptionIDList: number[] = this.valueOldForReceive.vacationOptionID;
  ...
  onAddNewVacationStatusID(event) {
    const itemValue: number = Number(event);
    this.valueOldForReceive = JSON.parse(JSON.stringify(this.formSearch.value));
    let newVacationStatusIDList: number[] = this.valueOldForReceive.vacationStatusID;
  ...
  onAddNewDepartmentID(event) {
    const itemValue: number = Number(event);
    this.valueOldForReceive = JSON.parse(JSON.stringify(this.formSearch.value));
    let newDepartmentIDList: number[] = this.valueOldForReceive.departmentID;
