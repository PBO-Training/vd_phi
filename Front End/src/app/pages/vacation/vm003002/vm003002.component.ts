...
import { ServerErrors } from './Vm003002';
...
@Component({
  selector: 'app-Vm003002',
  templateUrl: './Vm003002.component.html',
  styleUrls: ['./Vm003002.component.scss']
})
export class Vm003002Component implements OnInit, OnDestroy {
  ...
  constructor(
    private Vm003002Service: Vm003002Service,
  ...
  initDataLoad(id) {
    this.Vm003002Service.Vm003002InitList().pipe(takeUntil(this.unsubscribe$)).subscribe((data: any) => {
      if (data.error === null) {
  ...
  initDetailData(id) {
    ...
    if (typeof isNum === 'number' && (isNum % 1) === 0) {
      this.Vm003002Service.Vm003002getHistoryDetailVacation(id, this.accessEmployeeID).pipe(takeUntil(this.unsubscribe$)).subscribe((value) => {
        if (value) {
  ...
  back(event: any) {
    ...
    this.router.navigate(['/vacation-history-list']);
  }
