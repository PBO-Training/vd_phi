...
import { Role } from './ms018001-entity';
import { RoleRequest } from './ms018001-request';
import { MS018001Service } from './ms018001.service';
...
@Component({
  selector: 'app-ms018001',
  templateUrl: './ms018001.component.html',
  entryComponents: [ModalConfirmComponent],
})
export class MS018001Component implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    private roleService: MS018001Service,
  ...
  onSort({ column, direction }: SortEvent) {
    ...
    this.roleList = [...this.roleList].sort((a, b) => {
      ...
      switch (column) {
        case 'roleCode':
          res = compare(a.roleCode, b.roleCode);
          return direction === 'asc' ? res : -res;
        case 'roleName':
          res = compare(escape(a.roleName), escape(b.roleName));
          return direction === 'asc' ? res : -res;
      }
    });
  }
  ...
  private initData(pageN: any, pSize: any, formSearch: any): void {
    ...
    this.roleService.getRole(this.bodyRequest).pipe(takeUntil(this.unsubscribe$)).subscribe(resp => {
      if (resp.content.data !== undefined && resp.content.data !== null && resp.content.data.length >= 0) {
        ...
        this.roleList = resp.content.data;
  ...
  searchRole() {
    ...
    this.baseForm.value.roleName = this.valueOld.roleName;
    this.baseForm.value.roleCode = this.valueOld.roleCode;
  ...
  refreshCheckedStatus(): void {
    this.checked = this.roleList.every(item => this.screenProps.setOfCheckedId.has(+item.roleID));
    this.indeterminate = this.roleList.some(item => this.screenProps.setOfCheckedId.has(+item.roleID)) && !this.checked;
  }
  ...
  openPopup(ids: number[]) {
    ...
    modalRef.result.then(result => {
      if (result === 'delete') {
        this.roleService.deleteRole(ids).pipe(takeUntil(this.unsubscribe$)).subscribe(
          (resp) => {
  ...
  checkAll(event) {
    this.roleList.forEach(item => {
      this.updateCheckedSet(+item.roleID, event.target.checked);
    });
  }
  ...
  unCheckAll() {
    this.checked = false;
    this.roleList.forEach(item => {
      this.screenProps.setOfCheckedId.delete(+item.roleID);
    });
  }
