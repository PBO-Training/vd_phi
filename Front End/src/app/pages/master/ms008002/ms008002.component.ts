...
import { MS008002Service } from './ms008002.service';
...
@Component({
  selector: 'app-ms008002',
  templateUrl: './ms008002.component.html'
})
export class MS008002Component implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    private statusEmployeeService: MS008002Service,
  ...
  initData() {
    ...
    if (id) {
      this.statusEmployeeService.getDetailStatusEmployee(id).pipe(takeUntil(this.unsubscribe$)).subscribe((value) => {
        if (value) {
          ...
          this.formData.patchValue({
            statusEmployeeID: value.content.statusEmployeeID,
            statusEmployeeName: value.content.statusEmployeeName,
            statusEmployeeCode: value.content.statusEmployeeCode
          });
  ...
  submitForm() {
    this.submitted = true;
    this.formData.controls.statusEmployeeName.setValue(this.formData.controls.statusEmployeeName.value.trim());
    this.formData.controls.statusEmployeeName.setValidators([Validators.required, Validators.maxLength(LengthConstant.MAX_LENGTH_NAME)]);
    this.formData.controls.statusEmployeeCode.setValue(this.formData.controls.statusEmployeeCode.value.trim());
    // tslint:disable-next-line:max-line-length
    this.formData.controls.statusEmployeeCode.setValidators([Validators.required, Validators.maxLength(LengthConstant.MAX_LENGTH_CODE), Validators.pattern(Regex.fieldCode)]);
    this.validator.updateTreeValidity(this.formData);
    if (!this.formData.invalid) {
      // if form have employee status ID => Update else Create
      if (this.f.statusEmployeeID.value) {
        this.statusEmployeeService.updateStatusEmployee(this.formData.value).pipe(takeUntil(this.unsubscribe$)).subscribe(resp => {
        ...
        this.statusEmployeeService.createStatusEmployee(this.formData.value).pipe(takeUntil(this.unsubscribe$)).subscribe(
          (resp) => {
            if (resp.body.error === null) {
              this.toastService.show('notification-message.save-success', { classname: 'bg-success text-light', delay: 3000 });
              this.formData.reset();
