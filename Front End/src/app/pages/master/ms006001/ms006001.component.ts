...
import { LevelLanguage } from './ms006001-entity';
import { LevelLanguageRequest } from './ms006001-request';
import { MS006001SearchService } from './ms006001.service';

@Component({
  selector: 'app-ms006001',
  templateUrl: './ms006001.component.html',
  entryComponents: [ModalConfirmComponent]
})
export class MS006001SearchComponent implements OnInit, OnDestroy {
  ...
  levelLanguageOld: LevelLanguage[] = [];
  bodyRequest: LevelLanguageRequest;
  levelLanguageList: LevelLanguage[];
  ...
  constructor(
    ...
    private levelLanguageService: MS006001SearchService,
    ...
    this.formSearch = this.fb.group({
      levelLanguageName: [null],
      levelLanguageCode: [null]
    });
  ...
  ngOnInit(): void {
    ...
    if (this.backFlag === true && data !== null) {
      if (data.data === null) {
        ...
        this.formSearch = this.fb.group({
          levelLanguageName: data.data.levelLanguageName,
          levelLanguageCode: data.data.levelLanguageCode
        });
  ...
  onSort({ column, direction }: SortEvent) {
    ...
    this.column = column;
    this.levelLanguageList = [...this.levelLanguageList].sort((a, b) => {
  ...
  private initData(pageN: any, pSize: any, formSearch: any): void {
    ...
    this.levelLanguageService.getLevelLanguage(this.bodyRequest).pipe(takeUntil(this.unsubscribe$)).subscribe(resp => {
      if (resp.content.data !== undefined && resp.content.data !== null && resp.content.data.length >= 0) {
        ...
        this.levelLanguageList = resp.content.data;
  ...
  searchLevelLanguage() {
    // this.baseForm = { ...this.formSearch.value };
    this.valueOld = JSON.parse(JSON.stringify(this.formSearch.value));
    this.baseForm.value.levelLanguageName = this.valueOld.levelLanguageName;
    this.baseForm.value.levelLanguageCode = this.valueOld.levelLanguageCode;
  ...
  refreshCheckedStatus(): void {
    this.checked = this.levelLanguageList.every(item => this.screenProps.setOfCheckedId.has(+item.levelLanguageID));
    this.indeterminate = this.levelLanguageList.some(item => this.screenProps.setOfCheckedId.has(+item.levelLanguageID)) && !this.checked;
  }
  ...
  openPopup(ids: number[]) {
    ...
    modalRef.result.then(result => {
      if (result === 'delete') {
        this.levelLanguageService.deleteLevelLanguage(ids).pipe(takeUntil(this.unsubscribe$)).subscribe(resp => {
          if (resp.content) {
  ...
  checkAll(event) {
    this.levelLanguageList.forEach(item => {
      this.updateCheckedSet(+item.levelLanguageID, event.target.checked);
    });
  }
  ...
  unCheckAll() {
    this.checked = false;
    this.levelLanguageList.forEach(item => {
      this.screenProps.setOfCheckedId.delete(+item.levelLanguageID);
    });
  }
